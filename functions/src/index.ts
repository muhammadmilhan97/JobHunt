 ...
 */
export const signCloudinaryUpload = functions.https.onRequest(async (req, res) => {
  // Enable CORS
...
export const onApplicationUpdate = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    const newStatus = after.status as ApplicationStatus;
    ...
    try {
      ...
      const template = NOTIFICATION_TEMPLATES.APPLICATION_STATUS;
      await admin.firestore()
        .collection("users")
        .doc(jobSeekerId)
        ...
        .add({
          ...
          body: template.getBody(jobTitle, APPLICATION_STATUS_LABELS[newStatus] || newStatus),
          data: {
      ...
      await fcmService.sendApplicationStatusUpdate(
        jobSeekerId,
        jobTitle,
        APPLICATION_STATUS_LABELS[newStatus] || newStatus,
        applicationId,
        jobId
      );
      ...
      if (userEmail) {
        ...
        await mailerService.sendApplicationStatusUpdate(
          userEmail,
          userName,
          jobTitle,
          companyName,
...
export const onJobWrite = functions.firestore
  ...
  .onWrite(async (change, context) => {
    ...
    const isUpdatedJob = change.before.exists && 
      change.before.data()?.updatedAt !== after.updatedAt;
    ...
    try {
      ...
      // Process in batches
      const batchSize = 10;
      const template = NOTIFICATION_TEMPLATES.JOB_MATCH;
      ...
      for (let i = 0; i < matchingUsers.length; i += batchSize) {
        ...
        const emailPromises = batch
          ...
          .map(user => 
            mailerService.sendJobMatchNotification(
              user.data.email,
              user.data.name || "User",
              jobTitle,
              companyName,
              jobId,
              jobCategory,
              `${after.locationCity}, ${after.locationCountry}`
            )
          );
...
export const digestWeekly = functions.pubsub
  ...
  .onRun(async () => {
    try {
      ...
      const jobsByCategory = recentJobs.reduce((acc: any, job: any) => {
      ...
      for (let i = 0; i < users.length; i += batchSize) {
        ...
        const digestPromises = batch.map(async (userDoc) => {
          ...
          const topJobs = categoryJobs.slice(0, 10).map((job: any) => ({
            id: job.id,
            title: job.title,
            company: job.company,
          ...
          // Send FCM notification
          const fcmService = FCMService.getInstance();
          await fcmService.sendWeeklyDigestNotification(
            userId,
            topJobs.length,
            userCategory
          );
          ...
          if (userEmail) {
            const mailerService = MailerService.getInstance();
            await mailerService.sendWeeklyDigest(
              userEmail,
              userName,
              topJobs,
              userCategory
